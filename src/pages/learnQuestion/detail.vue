    ...
    :scroll-with-animation="true">
          <view class="question-page">
            ...
            <view class="options-list">
              <view v-for="(option, optIndex) in question.options_json" :key="optIndex" class="option-item" :class="optionClass(question, option)" @tap="selectOption(optIndex)">
            ...
            <view class="key-point-section" v-if="mode == 'wrong' || question.showAnswer">
              <text class="key-point-text">💡 {{ question.key_point }}</text>
            </view>

            <!-- AI解释 - 只在答错时显示 -->
            <view class="ai-explanation" v-if="mode == 'wrong' || question.showAnswer">
            ...
            <!-- 统计信息 - 只在答错时显示 -->
            <view class="stats-container" v-if="mode == 'wrong' || question.showAnswer">
            ...
            <!-- 社区评论区 - 只在答错时显示 -->
            <view class="comments-section" v-if="mode == 'wrong' || question.showAnswer">
...
<script>
  import {startTrain, collectAdd, collectCancel, wrongAdd, recordAdd, getQuestionDetail} from '@/http/api/testQuestions.js'
  ...
  export default {
    ...
    methods: {
      ...
      selectOption(optionIndex) {
        if (this.question.showAnswer) return;

        // 选择选项
        this.question.selectedOption = this.question.options_json[optionIndex].key;
        this.question.showAnswer = true;

        // 判断是否答对
        const isCorrect = this.question.options_json[optionIndex].key == this.question.answer;
      ...
      // 判断是否答对
      isCorrectAnswer() {
        if (this.question.selectedOption === null) return false;
      ...
      queryPostList () {
        queryPostList({
          ...
          this.question.comments = res.data.list.data
          this.question.displayedComments = this.showAllComments ? this.question.comments : this.question.comments.slice(0, 3)
        })
      },
      // 设置错题记录
      wrongAdd () {
    ...
    onLoad(option) {
      this.mode = option.mode ? option.mode : 'wrong'
