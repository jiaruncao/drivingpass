<template>
  <div class="question_main" @scroll="handleScroll">
    ...
    <div class="question_info">
      ...
      <div class="info_option" v-if="startLearnQuestion.kind == 'SINGLE'">
        <div
          class="option_item"
          v-for="(item, i) in startLearnQuestion.options_json"
          :key="i"
      ...
      <template v-if="selectOption">
        <div class="your_select">
          ...
          <span class="success">{{
            ...
            // )
            startLearnQuestion.answer
          }}</span>
          Your choice：
          <span
            class="select"
            :style="{ color: startLearnQuestion.answer == selectOption.key ? '#419FFF' : '#FF6666' }"
            >{{
              String.fromCharCode(
                65 +
                  startLearnQuestion.options_json.findIndex(
                    (e) => e.key == selectOption.key
                  )
              )
            }}</span
          >
    ...
    <div class="bottom_nav">
      <div class="save_icon" @click="saveQuestion">
        <u-icon
          :name="startLearnQuestion.collected ? 'star-fill' : 'star'"
          size="20"
          :color="startLearnQuestion.collected ? '#ff9900' : '#000'"
        ></u-icon>
      </div>
      <div class="save_name" @click="saveQuestion">
        {{ startLearnQuestion.collected ? "Saved" : "Save" }}
      </div>
      <div class="success">
      ...
      <div style="display: flex" @click="findAllQuestion">
    ...
    >
      <div class="setting_main">
        ...
        <div class="setting_option">
          <div class="option_name">Automatic Reading</div>
          <div class="option_switch">
            <u-switch
              v-model="settingQuery.automaticReading"
              @change="change"
            ></u-switch>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      commentShow: false,
      settingShow: false,
      settingQuery: { autonext: false, streakCelebration: false, automaticReading: false },
      replyQuery: {
        content: "",
      },
      questionI: 0,
  ...
  methods: {
    startTrain () {
      startTrain({
        ...
        console.log(res)
        // 获取第一题
        this.startLearnQuestion = res.data.data[0]
      })
    },
    /*
     * author：Gengbaodada
     ...
     */
    saveQuestion() {
      if (!this.startLearnQuestion.collected) {
        collectAdd({
          question_id: this.startLearnQuestion.id
        }).then(res => {
          console.log(res)
          this.startLearnQuestion.collected = true;
        ...
        collectCancel({
          ...
          console.log(res)
          this.startLearnQuestion.collected = false;
    ...
    handleOptionStyleText(item, i) {
      if (this.selectOption) {
        if (this.selectOption.key == item.key) {
          return this.selectOption.key == this.startLearnQuestion.answer ? "✔" : "×";
        } else {
          return item.key == this.startLearnQuestion.answer ? "✔" : String.fromCharCode(65 + i);
        }
      } else {
    ...
    handleOptionStyleBg(item) {
      if (this.selectOption) {
        if (this.selectOption.key == item.key) {
          ...
          return item.key == this.startLearnQuestion.answer
            ? "background: #419FFF; color: #fff"
            : "color: #000";
        }
      } else {
    ...
    handleOptionStyle(item, i) {
      if (this.selectOption) {
        if (this.selectOption.key == item.key) {
          return this.selectOption.key == this.startLearnQuestion.answer ? "color: #419FFF" : "color: #FF6666";
        } else {
          return item.key == this.startLearnQuestion.answer ? "color: #419FFF" : "";
        }
      } else {
        return "";
      }
