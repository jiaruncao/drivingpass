    ...
    <view class="container">
      ...
      <view class="media-section">
        <view class="media-grid">
          ...
          <!-- 添加视频按钮 -->
          <view v-if="!uploadedVideo && uploadedImages.length === 0" class="media-item add-media" @tap="triggerVideoUpload">
            <view class="add-icon">📹</view>
    ...
    <view v-if="showTestCentrePicker" class="modal-backdrop" @tap="closeTestCentrePicker">
      <view class="modal-content" @tap.stop>
        ...
        <scroll-view class="category-list" scroll-y>
          ...
          <view 
            v-for="(centre, index) in testCentres" 
            :key="index"
            class="category-option"
            :class="{selected: selectedTestCentre === centre.id}"
            @tap="chooseTestCentre(centre)">
...
<script>
import {addTag, queryTags, getCenterIndex,createPost} from '@/http/api/community.js'
export default {
  data() {
    return {
      // 帖子内容
      ...
      // 分类数据
      categories: [
        { id: 'theory_test', name: 'Theory Test', icon: '📚' },
        { id: 'hazard_perception', name: 'Hazard Perception', icon: '⚠️' },
        { id: 'practical_test', name: 'Practical Test', icon: '🚗' },
        { id: 'tips_tricks', name: 'Tips & Tricks', icon: '💡' },
