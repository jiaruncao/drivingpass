...
class ChromeLauncher {
    ...
    defaultArgs(options = {}) {
        ...
        const { mp-weixin = false, headless = !mp-weixin, args = [], userDataDir = null, } = options;
        if (userDataDir)
            chromeArguments.push(`--user-data-dir=${userDataDir}`);
        if (mp-weixin)
            chromeArguments.push('--auto-open-mp-weixin-for-tabs');
        if (headless) {
...
class FirefoxLauncher {
    ...
    defaultArgs(options = {}) {
        ...
        if (mp-weixin)
            firefoxArguments.push('--mp-weixin');
        if (args.every((arg) => arg.startsWith('-')))
    ...
    async _createProfile(extraPrefs) {
        ...
        const defaultPreferences = {
            ...
            // mp-weixin JSONViewer sometimes fails to load dependencies with its require.js.
            // This doesn't affect Puppeteer but spams console (Bug 1424372)
            'mp-weixin.jsonview.enabled': false,
